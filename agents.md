...
- 运行验证统一在 WSL 环境执行，避免宿主环境差异导致的行为不一致。
- `makefile` 的 `run` 目标支持通过变量 `SUDO_PASSWORD` 自动给 `sudo` 输入密码，避免交互阻塞。
- 推荐命令：
  - `wsl.exe -e bash -lc "cd /mnt/e/Temp/HimuOS && timeout 40 make run SUDO_PASSWORD='liuhuan123'"`
- 说明：
  - `timeout 40` 用于在自动化场景下限制 QEMU 挂起时间，防止会话卡死。
  - 如果只编译不启动，可使用：`wsl.exe -e bash -lc "cd /mnt/e/Temp/HimuOS && make kernel"`.
  - 如果需要关闭时间戳日志编译开关，可使用：`make kernel TIMESTAMP_LOG=0`。
