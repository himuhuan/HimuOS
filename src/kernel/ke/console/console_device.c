 ...
 * HimuOperatingSystem
 *
 * File: ke/console_device.c
 * Description:
 * Ke Layer - Text console device implementation
...
static BOOL TryProcessAnisSeq(KE_CONSOLE_DEVICE *dev, char ch);

// Map ANSI code to console device attributes.
static void ApplyAnsiCode(KE_CONSOLE_DEVICE *dev, int code);
...
HO_KERNEL_API int
KeConDevPutChar(KE_CONSOLE_DEVICE *this, char c)
{
...
KeConDevPutStr(KE_CONSOLE_DEVICE *this, const char *str)
{
    uint64_t written = 0;
    const char *p;
    ...
    for (p = str; *p; ++p)
    {
        int code = KeConDevPutChar(this, *p);
        if (code != EC_SUCCESS)
            break;
        ++written;
    }
...
HO_KERNEL_API void
KeConDevClearScreen(KE_CONSOLE_DEVICE *dev, COLOR32 color)
{
...
static BOOL
TryProcessAnisSeq(KE_CONSOLE_DEVICE *dev, char ch)
{
...
static void
ApplyAnsiCode(KE_CONSOLE_DEVICE *dev, int code)
{
