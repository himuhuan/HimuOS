 ...
 *
 * File: ke/user_mode.c
 * Description: Minimal staged user-mode mapping and first-entry helpers.
...
#define KE_USER_MODE_MAX_MAPPINGS 3U
#define KE_USER_MODE_INITIAL_RFLAGS 0x202ULL

typedef enum KE_USER_MODE_MAPPING_KIND
{
    KE_USER_MODE_MAPPING_KIND_CODE = 0,
    KE_USER_MODE_MAPPING_KIND_CONST,
    KE_USER_MODE_MAPPING_KIND_STACK,
} KE_USER_MODE_MAPPING_KIND;

typedef struct KE_USER_MODE_MAPPING_RECORD
{
    KE_USER_MODE_MAPPING_KIND Kind;
    HO_VIRTUAL_ADDRESS VirtualBase;
    HO_PHYSICAL_ADDRESS PhysicalBase;
    uint64_t Attributes;
} KE_USER_MODE_MAPPING_RECORD;
...
struct KE_USER_MODE_STAGING
{
    ...
    KE_USER_MODE_MAPPING_RECORD Mappings[KE_USER_MODE_MAX_MAPPINGS];
};

extern HO_NORETURN void KiUserModeIretq(HO_VIRTUAL_ADDRESS userRip,
...
static HO_STATUS KiValidateCreateParams(const KE_USER_MODE_CREATE_PARAMS *params);
static HO_STATUS KiValidateUserModeHole(const KE_KERNEL_ADDRESS_SPACE *space, HO_VIRTUAL_ADDRESS virtAddr);
static HO_STATUS KiPopulatePhysicalPage(HO_PHYSICAL_ADDRESS physAddr, const void *bytes, uint64_t byteCount);
static HO_STATUS KiRecordMappedPage(KE_USER_MODE_STAGING *staging,
                                    KE_USER_MODE_MAPPING_KIND kind,
...
static HO_STATUS KiAllocateAndMapUserPage(const KE_KERNEL_ADDRESS_SPACE *space,
                                          ...
                                          const void *bytes,
                                          uint64_t byteCount);
static const KE_USER_MODE_MAPPING_RECORD *KiFindMappedPage(const KE_USER_MODE_STAGING *staging,
                                                                KE_USER_MODE_MAPPING_KIND kind);
static KE_USER_MODE_STAGING *KiGetCurrentThreadStaging(void);
...
static HO_STATUS
KiValidateCreateParams(const KE_USER_MODE_CREATE_PARAMS *params)
{
    if (!params)
...
static HO_STATUS
KiValidateUserModeHole(const KE_KERNEL_ADDRESS_SPACE *space, HO_VIRTUAL_ADDRESS virtAddr)
{
    if (!space)
        return EC_ILLEGAL_ARGUMENT;
...
static HO_STATUS
KiRecordMappedPage(KE_USER_MODE_STAGING *staging,
                   KE_USER_MODE_MAPPING_KIND kind,
                   HO_VIRTUAL_ADDRESS virtAddr,
                   HO_PHYSICAL_ADDRESS physAddr,
                   uint64_t attributes)
{
    if (!staging)
        return EC_ILLEGAL_ARGUMENT;
    if (staging->MappingCount >= KE_USER_MODE_MAX_MAPPINGS)
        return EC_INVALID_STATE;

    KE_USER_MODE_MAPPING_RECORD *record = &staging->Mappings[staging->MappingCount++];
    record->Kind = kind;
...
static const KE_USER_MODE_MAPPING_RECORD *
KiFindMappedPage(const KE_USER_MODE_STAGING *staging, KE_USER_MODE_MAPPING_KIND kind)
{
    if (!staging)
        return NULL;
    ...
    {
        const KE_USER_MODE_MAPPING_RECORD *record = &staging->Mappings[index];
        if (record->Kind == kind)
            return record;
    }
...
KiGetCurrentThreadStaging(void)
{
    ...
    KE_USER_MODE_STAGING *staging = ExUserRuntimeQueryThreadStaging(thread);
    if (staging == NULL)
        return NULL;
...
KeUserModeCreateStaging(const KE_USER_MODE_CREATE_PARAMS *params,
                             ...
                             KE_USER_MODE_STAGING **outStaging)
{
    ...
    *outStaging = NULL;
    ...
    status = KiValidateUserModeHole(space, 0);
    if (status != EC_SUCCESS)
        return status;

    status = KiValidateUserModeHole(space, EX_USER_IMAGE_CODE_BASE);
    if (status != EC_SUCCESS)
        return status;

    status = KiValidateUserModeHole(space, EX_USER_IMAGE_CONST_BASE);
    if (status != EC_SUCCESS)
        return status;

    status = KiValidateUserModeHole(space, EX_USER_IMAGE_STACK_GUARD_BASE);
    if (status != EC_SUCCESS)
        return status;

    status = KiValidateUserModeHole(space, EX_USER_IMAGE_STACK_BASE);
    if (status != EC_SUCCESS)
        return status;

    KE_USER_MODE_STAGING *staging = (KE_USER_MODE_STAGING *)kzalloc(sizeof(*staging));
    if (!staging)
    ...
    status = KiAllocateAndMapUserPage(space,
                                      staging,
                                      KE_USER_MODE_MAPPING_KIND_CODE,
...
cleanup:
    destroyStatus = KeUserModeDestroyStaging(staging);
...
HO_KERNEL_API HO_NODISCARD HO_STATUS
KeUserModeDestroyStaging(KE_USER_MODE_STAGING *staging)
{
    if (!staging)
        return EC_ILLEGAL_ARGUMENT;
    ...
    {
        KE_USER_MODE_MAPPING_RECORD *record = &staging->Mappings[staging->MappingCount - 1];
...
HO_KERNEL_API HO_NODISCARD HO_STATUS
KeUserModeAttachThread(KTHREAD *thread, KE_USER_MODE_STAGING *staging)
{
    if (!thread || !staging)
        return EC_ILLEGAL_ARGUMENT;
    ...
    if (staging->AttachedThread != NULL && staging->AttachedThread != thread)
        return EC_INVALID_STATE;
    if (KiFindMappedPage(staging, KE_USER_MODE_MAPPING_KIND_CODE) == NULL)
        return EC_INVALID_STATE;
    if (KiFindMappedPage(staging, KE_USER_MODE_MAPPING_KIND_STACK) == NULL)
        return EC_INVALID_STATE;
...
HO_KERNEL_API HO_NODISCARD HO_STATUS
KeUserModeDetachThread(KTHREAD *thread, KE_USER_MODE_STAGING *staging)
{
    if (!thread || !staging)
        return EC_ILLEGAL_ARGUMENT;
    if (staging->AttachedThread == NULL)
...
HO_KERNEL_API HO_NODISCARD HO_STATUS
KeUserModePatchConstBytes(KE_USER_MODE_STAGING *staging,
                               uint64_t offset,
                               const void *bytes,
                               uint64_t length)
{
    ...
    const KE_USER_MODE_MAPPING_RECORD *constRecord =
        KiFindMappedPage(staging, KE_USER_MODE_MAPPING_KIND_CONST);
    if (constRecord == NULL)
        return EC_INVALID_STATE;
...
HO_KERNEL_API HO_NODISCARD HO_STATUS
KeUserModeQueryCurrentThreadLayout(KE_USER_MODE_LAYOUT *outLayout)
{
    if (!outLayout)
        return EC_ILLEGAL_ARGUMENT;
    ...
    const KE_USER_MODE_MAPPING_RECORD *codeRecord =
        KiFindMappedPage(staging, KE_USER_MODE_MAPPING_KIND_CODE);
...
KeUserModeEnterCurrentThread(void)
{
    ...
    HO_KASSERT(KiFindMappedPage(staging, KE_USER_MODE_MAPPING_KIND_CODE) != NULL, EC_INVALID_STATE);
