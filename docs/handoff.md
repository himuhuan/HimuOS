...
需要额外对齐的一点是：当前主线 MVP 已不再按“完整用户子系统”推进，而是先收口为一个 `demo-shell vertical slice`。也就是说，官方目标是内核 boot 后直接拉起用户态 `hsh`，并围绕 `tick1s` / `calc` / `kill` / `help` / `exit` 这组受限命令完成演示；它明确排除文件系统、PATH 搜索、通用 loader、内核态 shell 与独立 PID allocator。
...
| 调度器 | 优先级感知的时间片轮转（RR），10ms 量子，抢占式，tickless；ready queue 已按优先级分层 |
...
| P3 exit/reap 合同 | thread-terminated → finalizer teardown → idle/reaper reclaim 证据链 |
...
- `scripts/qemu_capture.sh` 现在支持 `QEMU_CAPTURE_MODE=host|tcg|custom`；默认值仍是 `host`，但 `make run` 已不再把 KVM 路径硬编码为唯一执行模型。
- 无参 `make run` / `make iso` / `make run_iso` 现在默认装配 `demo_shell` 作为交互入口，并落在独立的 `default-demo_shell` build flavor；手工输入应发生在 QEMU GTK 窗口内，宿主终端上的 `-serial stdio` 仅承载串口输出。
- 为避免 GTK 交互时被运行时日志持续刷屏，`make run` / `make iso` / `make run_iso` 在 `QEMU_DISPLAY=gtk` 下默认把最小日志等级提升到 `WARNING`；因此 `[DBG]` 与 `[INF]` 默认都不会进入 GTK 交互输出，而无头 `qemu_capture.sh` 路径仍保留完整诊断日志。
- 对 `user_dual` 以及其他 teardown / timing-sensitive profile，单份 host/KVM 串口日志不再视为充分证据；必须同时保留 `host` 与 `tcg` 两条路径的捕获结果。
- 如果 host 与 TCG 的结果不一致，必须在 bug 记录、handoff 或回归说明中显式标注分歧，而不是把 host/KVM 成功误记为“问题不存在”。
...
以下功能是 `Readme.md` 愿景明确承诺、但当前尚未完整实现的。需要特别注意的是：2026-04-05 合入主线的 PR `#39` / `#40` / `#41` 已经把 Ex 对象、私有 handle table 与 capability syscall 的 bootstrap-only 前置工作带进 `main`，因此这些条目不应再被描述为“完全未开始”，而应理解为“已有 pilot / groundwork，但离正式功能仍有明显差距”。同时，当前官方实施主线已经收口为 `demo-shell vertical slice`，因此这里的优先顺序不应再把“优先级调度增强”误读为下一阶段前置项。

### 4.1 优先级调度（Priority Scheduling）

- **愿景**：README 承诺"基于优先级和时间片轮转（RR）的调度器"
- **现状**：当前代码已经具备多级 ready queue、最高优先级选取与优先级感知的 RR 调度路径；因此它不再是 `demo-shell` 主线的核心缺口
- **缺口**：若后续要继续增强，主要会落在策略调优与更完整的优先级语义，而不是从零开始补 ready queue
- **估计复杂度**：低-中，但当前官方主线不再把它列为下一阶段前置项
...
### 4.6 demo-shell vertical slice / 受限用户程序模型

- **愿景**：当前官方主线是 `demo-shell vertical slice`，而不是完整用户子系统；目标是以 `hsh` 为用户态入口，冻结命令面为 `tick1s` / `calc` / `kill` / `help` / `exit`
- **现状**：已有 `user_hello` / `user_caps` / `user_dual` 等 bootstrap-only 地基，编译型用户程序嵌入与最小 syscall / handle 路径已经成立，但 `hsh` 这条官方演示切片尚未落地
- **缺口**：最小键盘输入链路、demo-scope 进程控制面、`hsh` / `tick1s` / `calc` 的受限行为，以及保守的 `kill <pid>` 演示语义
- **完成度判断**：地基已成立，当前需要先把范围冻结为 demo-scope 合同，再按 `P0 -> P1 -> P2 -> P3 -> P4` 串行推进
- **估计复杂度**：中-高，但边界比“完整用户态子系统”窄得多
- **依赖**：4.2 键盘输入，以及现有 bootstrap 用户态 / capability 基础继续复用
...
> HimuOS 已经完成了一个**架构成熟、机制完整的 Ke 层教学内核**——从 UEFI 启动到虚拟内存、线程调度、同步原语、诊断观测一应俱全——并且在 bootstrap-only 用户态上继续向前推进：截至 PR `#39` / `#40` / `#41`，Ex 最小对象核心、进程私有 handle table 与 capability syscall pilot 已经进入 `main`。这意味着 Object/Capability/Syscall 不再是“从零开始”，但它们仍只覆盖 bootstrap 切片；当前官方主线应先把这些地基收口成 `demo-shell vertical slice`，优先补齐键盘输入这条硬前置，再串行推进 `hsh` / `tick1s` / `calc` / `kill` 这条受限演示路径。
